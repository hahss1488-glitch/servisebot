...
APP_VERSION = "2026.02.10-hotfix-1"
...
async def menu_command(update: Update, context: CallbackContext):
    ...
    has_active = DatabaseManager.get_active_shift(db_user['id']) is not None
    await update.message.reply_text(
        f"Меню открыто. Версия: {APP_VERSION}",
        reply_markup=create_main_reply_keyboard(has_active)
    )

async def version_command(update: Update, context: CallbackContext):
    await update.message.reply_text(f"Текущая версия бота: {APP_VERSION}")
...
async def handle_message(update: Update, context: CallbackContext):
    ...
    if context.user_data.get('awaiting_distance') and text in {
        MENU_OPEN_SHIFT,
        MENU_ADD_CAR,
        MENU_CURRENT_SHIFT,
        MENU_HISTORY,
        ...
        context.user_data.pop('awaiting_distance', None)
        await update.message.reply_text("Ок, ввод километража отменён.")
