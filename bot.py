...
def normalize_car_number(text: str) -> str:
    """Нормализация номера машины (правильные буквы!)"""
    text = text.strip().upper()
    
    # Заменяем английские на русские (только разрешённые)
    eng_to_rus = {
        'A': 'А', 'B': 'В', 'C': 'С', 'E': 'Е', 'H': 'Н',
        'K': 'К', 'M': 'М', 'O': 'О', 'P': 'Р', 'T': 'Т',
        'X': 'Х', 'Y': 'У'
    ...
    # Удаляем все не разрешённые символы
    normalized = re.sub(f'[^{ALLOWED_LETTERS}0-9]', '', normalized)
    
    # Автодобавление региона если не хватает
    if len(normalized) <= 6:
        normalized += "797"
    
    return normalized

def validate_car_number(text: str) -> Tuple[bool, str]:
    ...
    # Проверяем формат: буква-3 цифры-2 буквы-3 цифры
    pattern = f'^[{ALLOWED_LETTERS}]{{1}}\\d{{3}}[{ALLOWED_LETTERS}]{{2}}\\d{{3}}$'
...
async def handle_message(update: Update, context: CallbackContext):
    ...
    # Ожидание номера машины
        ...
        is_valid, result = validate_car_number(text)
        
        if not is_valid:
            await update.message.reply_text(
                f"❌ **Неправильный номер!**\n\n"
